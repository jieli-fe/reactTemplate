...
ZENG.msgbox._setPosition = function(tips, timeout, topPosition,callback) {
    ...
    timeout && (_s._timer = setTimeout(_s.hide(timeout,callback), timeout));
};
ZENG.msgbox.hide = function(timeout,callback) {
	return function()
    {
	    var _s = ZENG.msgbox;
	    if (timeout) {
	        clearTimeout(_s._timer);
	        _s._timer = setTimeout(_s._hide(callback));
	    } else {
	        _s._hide();
	    }
    }
};
ZENG.msgbox._hide = function(callback) {
	if(callback==null){
